...
def parse_battlescribe_catalogue(xml_file_path):
    ...
    for entry in root.findall(f".//{namespace}selectionEntry[@type='unit']"):
        ...
        for profile in profiles:
            ...
            # Add unit data with faction as first column
            unit_data = {
    ...
    if not weapons_df.empty:
        weapons_df = weapons_df.drop_duplicates(subset=['faction', 'weapon_name', 'weapon_type', 'range', 'attacks', 'skill', 'strength', 'ap', 'damage'])
    if not abilities_df.empty:
        abilities_df = abilities_df.drop_duplicates(subset=['faction', 'ability_name', 'description'])
...
if __name__ == "__main__":
    ...
    repository_path = "C:\\stentor\\wh40\\wh40k-10e"  # Adjust this path as needed
    
    if not os.path.exists(repository_path):
        print(f"Repository path not found: {repository_path}")
        print("Please clone the repository first:")
        print("git clone https://github.com/BSData/wh40k-10e.git")
        exit(1)
