...
def parse_battlescribe_catalogue(xml_file_path):
    ...
    for entry in root.findall(f".//{namespace}selectionEntry[@type='unit']"):
        ...
        for profile in profiles:
            ...
            if costs:
                unit_data['points_cost'] = convert_to_number(get_element_attrib(costs[0], 'value'))
            
            units_data.append(unit_data)
        
        # Extract weapons from this unit
        extract_weapons_from_unit(entry, unit_id, faction_name, weapons_data, unit_weapons_data, namespace)
        
        # Extract abilities from this unit
        extract_abilities_from_unit(entry, unit_id, faction_name, abilities_data, unit_abilities_data, namespace)
...
def extract_weapons_from_unit(unit_entry, unit_id, faction_name, weapons_data, unit_weapons_data, namespace):
    ...
    for weapon_profile, weapon_type in all_weapons:
        ...
        weapon_data = {
            ...
            'skill': convert_to_number(characteristics.get('WS' if weapon_type == 'Melee' else 'BS', '')),
...
def extract_abilities_from_unit(unit_entry, unit_id, faction_name, abilities_data, unit_abilities_data, namespace):
...
def remove_duplicates_from_tables(dataframes_dict):
    ...
    duplicate_criteria = {
        'units': ['faction', 'unit_name', 'profile_name', 'movement', 'toughness', 'save', 'wounds', 'leadership', 'objective_control'],
        'weapons': ['faction', 'weapon_name', 'weapon_type', 'range', 'attacks', 'skill', 'strength', 'ap', 'damage', 'keyword01', 'keyword02', 'keyword03', 'keyword04', 'keyword05'],
    ...
    for table_name, df in dataframes_dict.items():
        if not df.empty and table_name in duplicate_criteria:
            ...
            available_columns = [col for col in duplicate_criteria[table_name] if col in df.columns]
            ...
            if available_columns:
                ...
                print(f"{table_name.upper()}: {original_count} → {len(df_cleaned)} records ({duplicates_removed} duplicates removed)")
                
                cleaned_data[table_name] = df_cleaned
            else:
                print(f"{table_name.upper()}: No duplicate criteria columns found, keeping original")
...
def process_all_factions(repository_path):
    ...
    if all_unit_abilities:
        combined_data['unit_abilities'] = pd.concat(all_unit_abilities, ignore_index=True)
    else:
...
if __name__ == "__main__":
    ...
    repository_path = "C:\\stentor\\wh40\\wh40k-10e"  # Your specified path
